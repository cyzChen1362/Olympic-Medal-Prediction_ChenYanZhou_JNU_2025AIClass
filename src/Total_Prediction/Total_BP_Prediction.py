...
model = MLPRegressor(
    hidden_layer_sizes=(64, 32), # 两层隐藏层，单层可以如(64,)
    activation='relu',           # 常用激活函数
    solver='adam',               # 优化算法
    learning_rate='adaptive',    # 自适应学习率
    max_iter=1000,               # 迭代次数，可酌情增加
    random_state=7
)
...
# 9. 全部预测，并写入新表格
df['Pred_Total'] = model.predict(scaler.transform(X))
save_path = r'C:\Users\cyz13\PycharmProjects\AI_ClassProject\src\Total_Prediction\Total_BP_Prediction_Data_with_Pred.xlsx'
