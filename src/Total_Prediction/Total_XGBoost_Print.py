...
plt.ylabel('Values')
plt.title('XGBoost Prediction Results with Confidence Intervals')
plt.grid(True)
...
# 2. 获取真实值和预测值
true_values = df['Total'].values
pred_values = df['Pred_Total'].values

# 3. 绘图
plt.figure(figsize=(10, 6))
plt.plot(true_values, label='Ground Truth', color='blue', linewidth=1)
plt.plot(pred_values, label='Predicted Values', color='green', linewidth=1)
plt.xlabel('Sample Index')
plt.ylabel('Values')
